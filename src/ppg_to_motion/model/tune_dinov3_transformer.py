...
---------------------
  [B, 3, F_in, T_in]                   (stft_image, pre-normalised to ImageNet stats)
    ...
    → transformer decoder              learnable queries [B, T_patches, C] cross-attend to patch tokens
    → linear output head               [B, T_patches, samples_per_patch]
...
class DinoAccelReconstructor(nn.Module):
    """
    DINOv3 ViT-B/16 backbone with LoRA adapters and a transformer decoder that
    reconstructs a 1-D accelerometer magnitude signal via cross-attention over
    all backbone patch tokens.
    ...
    decoder_hidden      : feedforward dim inside each TransformerDecoderLayer.
    decoder_layers      : number of TransformerDecoderLayer blocks.
    decoder_dropout     : dropout used inside the transformer decoder.
    """
    ...
    def __init__(
        ...
        # ── Transformer Decoder ───────────────────────────────────────────────
        # Learnable query tokens — one per temporal output patch (T_patches = ACC_LEN // samples_per_patch)
        T_patches = ACC_LEN // samples_per_patch
        self.query_embed = nn.Parameter(torch.randn(1, T_patches, self.hidden))
        ...
        # Final projection from hidden dim to raw accelerometer values per patch
        self.output_head = nn.Linear(self.hidden, samples_per_patch)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        patch_tokens = tokens[:, self.num_prefix_tokens:, :]             # [B, N, C]  (memory)

        # Expand learnable queries to batch size
        queries = self.query_embed.expand(B, -1, -1)                     # [B, T_patches, C]

        # Transformer decoding: queries cross-attend to all backbone patch tokens
        decoded = self.transformer_decoder(tgt=queries, memory=patch_tokens)  # [B, T_patches, C]

        # Project each decoded token to a chunk of acc samples
        out = self.output_head(decoded)   # [B, T_patches, samples_per_patch]
        return out.reshape(B, -1)         # [B, T_patches × samples_per_patch]
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--decoder-hidden",   type=int,   default=512,
                   help="Feedforward dim inside each TransformerDecoderLayer")
    p.add_argument("--decoder-layers",   type=int,   default=2,
                   help="Number of TransformerDecoderLayer blocks")
    p.add_argument("--decoder-dropout",  type=float, default=0.1,
                   help="Dropout used inside the transformer decoder")
